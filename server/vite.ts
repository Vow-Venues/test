...
export async function setupVite(app: Express, server: Server) {
  ...
  // Serve static files from the hall pictures directory
  const hallPicturesPath = path.resolve(__dirname, "..", "client", "hall pictures data");
  app.use("/hall-pictures", express.static(hallPicturesPath, {
    setHeaders: (res, path) => {
  ...
  app.get('/api/venues/:id', async (req, res) => {
    try {
      ...
      if (!id || !mongoose.Types.ObjectId.isValid(id)) {
        log(`Invalid venue ID format: ${id}`);
